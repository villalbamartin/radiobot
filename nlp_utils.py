...
def reply(llm, initial_prompt, conversation_log, context_turns=5, username="User"):
...
def broadcast(llm, initial_prompt, speech_log, context_turns=10, username="User"):
    ...
    """
    prompt = initial_prompt.format(username=username)
    for utterance in conversation_log[-context_turns:]:
        prompt += "I: {utterance}\n"
    prompt += "I: "
    return prompt
